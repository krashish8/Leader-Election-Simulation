...
def send_broadcast_message(message, neighbour_id):
    ...
    for neighbour_port in neighbours_port:
        ...
        if flag == 0:
            sent_message = 1
            import copy
            temp_exc = copy.deepcopy(exclude_ids)
            for i, j in enumerate(neighbours_id):
                if (i >= 22):
                    break
                temp_exc.append(j)

            temp_exc = [i for n, i in enumerate(temp_exc) if i not in temp_exc[:n]]
            
            message = str(leader_id) + '|' + ','.join([str(i) for i in temp_exc])
